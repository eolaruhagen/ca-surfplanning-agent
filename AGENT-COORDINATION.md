...
- _2026-05-03 @UI_: **Live-feed refactored to panel-of-entities stage.** `live-feed.tsx` is now a 5-slot fixed layout: orchestrator at top-center, 4-column specialist grid below. `agent-card.tsx` rewritten as a stateful per-agent dashboard with 5-way `panelState` (idle/queued/thinking/action/done), character SVGs from v5 mockup in all five icon files, dashed consult-frame + ribbon for `consultedBy`, compact `tool-chip.tsx`. New `inter-agent-arc.tsx` renders SVG arcs for fresh Q/A/handoff messages (3.2s lifecycle). Hook additions: `activeConsultations` Map, `recentMessages` array with timestamps, richer `AgentMessage` type, `panelState` derivation. Demo stream gains a full consultation sequence (Planner→Recon Q/A over Mavericks). Tests: 125 → 187 (+62), all green. CLAUDE.md files added for live-feed and agent-icons. Commit: `f0f3af6`.
